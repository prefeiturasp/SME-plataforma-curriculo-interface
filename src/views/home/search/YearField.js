...
class YearField extends Component {
  ...
  render() {
    ...
    const word = numSelectedYears === 1 ? 'ano' : 'anos';

    let label = 'Todos os anos';
    if (numSelectedYears > 0) {
      const yearsLabels = selectedYears.map(year => year.description);
      const joinedLabels = yearsLabels.join(', ');
      label = `${joinedLabels} ${word}`;
    }
